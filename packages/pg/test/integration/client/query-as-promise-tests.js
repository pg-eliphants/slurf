...
suite.test('promise API with configurable promise type', (cb) => {
  const client = new pg.Client({ Promise: Promise })
  const connectPromise = client.connect()
  assert(connectPromise instanceof Promise, 'Client connect() returns configured promise')

  connectPromise
    .then(() => {
      const queryPromise = client.query('SELECT 1')
      assert(queryPromise instanceof Promise, 'Client query() returns configured promise')
